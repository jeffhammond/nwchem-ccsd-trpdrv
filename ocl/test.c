...
#ifdef USE_OPENCL
void ccsd_trpdrv_opencl_(double * restrict f1n, double * restrict f1t,
...
int main(int argc, char* argv[])
{
    ...
    if (argc<3) {
        printf("Usage: ./test nocc nvir [maxiter] [nkpass]\n");
        return argc;
    } else {
        ncor = 0;
        nocc = atoi(argv[1]);
    ...
    printf("Test driver with nocc=%d, nvir=%d, maxiter=%d, nkpass=%d\n", nocc, nvir, maxiter, nkpass);
...
#ifdef USE_OPENCL
                    ccsd_trpdrv_opencl_(f1n, f1t, f2n, f2t, f3n, f3t, f4n, f4t, eorb,
...
#else
                    ccsd_trpdrv_cbody_(f1n, f1t, f2n, f2t, f3n, f3t, f4n, f4t, eorb,
                                       &ncor, &nocc, &nvir, &emp4, &emp5, &a, &i, &j, &k, &klo,
                                       Tij, Tkj, Tia, Tka, Xia, Xka, Jia, Jka, Kia, Kka, Jij, Jkj, Kij, Kkj,
                                       dintc1, dintx1, t1v1, dintc2, dintx2, t1v2);
#endif
