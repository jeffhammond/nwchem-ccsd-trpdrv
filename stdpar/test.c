...
#ifdef USE_FORTRAN
void ccsd_trpdrv_acc_fbody_(double * restrict f1n, double * restrict f1t,
...
#else
void ccsd_trpdrv_acc_cbody_(double * restrict f1n, double * restrict f1t,
                            double * restrict f2n, double * restrict f2t,
                            double * restrict f3n, double * restrict f3t,
                            double * restrict f4n, double * restrict f4t,
                            double * restrict eorb,
...
{
    double * a;
    cudaMallocManaged((void**)&a, n*sizeof(double), 0x01);
    for (int i=0; i<n; i++) {
        a[i] = 1.0/(100.0+i);
    }
    return a;
...
#ifdef USE_FORTRAN
                    ccsd_trpdrv_acc_fbody_(f1n, f1t, f2n, f2t, f3n, f3t, f4n, f4t, eorb,
...
#else
                    ccsd_trpdrv_acc_cbody_(f1n, f1t, f2n, f2t, f3n, f3t, f4n, f4t, eorb,
                                           &ncor, &nocc, &nvir, &emp4, &emp5, &a, &i, &j, &k, &klo,
                                           Tij, Tkj, Tia, Tka, Xia, Xka, Jia, Jka, Kia, Kka, Jij, Jkj, Kij, Kkj,
                                           dintc1, dintx1, t1v1, dintc2, dintx2, t1v2);
#endif
